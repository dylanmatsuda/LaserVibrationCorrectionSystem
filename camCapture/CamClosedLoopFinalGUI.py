...
# PID Controllers
pid_x = PID(0.02, 0, 0.000, setpoint=0)
...
def process_frame(window):
    ...
    while processing:
        ...
        if elapsed_time > STEP_DELAY and not step_applied:
            ...
            window.update_correction_status("ACTIVE")
            window.update_setpoint_display(TARGET_X, TARGET_Y)
        ...
        with frame_lock:
            latest_frame = frame.copy()
            thresholded_frame = thresholded_bgr.copy()  # ✅ Prevent GUI race condition
    ...
    window.update_processing_status("IDLE")
...
class LaserTrackingApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.video_text.setStyleSheet("font-size: 24px; font-weight: bold; margin-top: 5px;")
        ...
        self.threshold_text.setStyleSheet("font-size: 24px; font-weight: bold; margin-top: 5px;")
        ...
        self.processing_status_label = QLabel("Processing Status: IDLE")
        ...
        self.processing_status_label.setStyleSheet("""
            ...
            color: gray;
        """)
        right_sidebar.addWidget(self.processing_status_label)
        ...
        # 🔹 **Setpoint Display**
        self.setpoint_label = QLabel("Setpoints: N/A")
        ...
        self.setpoint_label.setStyleSheet("""
            ...
            color: gray;
        """)
        right_sidebar.addWidget(self.setpoint_label)

        # 🔹 **Fixed Parameter Display (Camera Info)**
        self.system_info_label = QLabel(
            f"Camera Resolution: {int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))}\n"
        ...
        self.system_info_label.setStyleSheet("""
            ...
            color: black;
        """)
        right_sidebar.addWidget(self.system_info_label)
    ...
    def update_processing_status(self, status):
        """ Updates processing status dynamically. """
        color = "#155724" if status == "PROCESSING" else "gray"
        background = "#d4edda" if status == "PROCESSING" else "#f0f0f0"

        self.processing_status_label.setText(f"Processing Status: {status}")
        self.processing_status_label.setStyleSheet(f"""
            font-size: 22px;
            font-weight: bold;
            padding: 12px;
            border-radius: 8px;
    ...
    def update_oscillation_label(self):
        """ Updates the oscillation frequency display based on availability. """
    ...
    def update_setpoint_display(self, x_setpoint=None, y_setpoint=None):
        """ Updates the setpoint display based on calculation status. """
        if x_setpoint is None or y_setpoint is None:
            self.setpoint_label.setText("Setpoints: Calculating...")
            self.setpoint_label.setStyleSheet("""
                font-size: 22px;
                font-weight: bold;
                padding: 12px;
            ...
            self.setpoint_label.setText(f"Setpoints: X = {x_setpoint:.1f} px, Y = {y_setpoint:.1f} px")
            self.setpoint_label.setStyleSheet("""
                font-size: 22px;
                font-weight: bold;
                padding: 12px;
    ...
    def start_processing(self):
        """ Start frame processing only if PID and Timing inputs are valid. """
        global processing
        if not processing:
            try:
                self.update_pid()  # ✅ Validate and update PID values
                self.update_timing()  # ✅ Validate and update timing values
            except ValueError as e:
                self.show_error_message(f"Input Error: {str(e)}")
                return  # ✅ Stop processing if invalid values

            self.update_processing_status("PROCESSING")
            self.start_button.setEnabled(False)  # 🔹 Disable button while processing
            processing = True  # 🔹 Set processing flag
            processing_thread = threading.Thread(target=self.process_frame_with_ui_update, daemon=True)
